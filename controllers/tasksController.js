...
async function completeTask(req, res, next) {
  try {
    ...
    res.json({
      message: `Task id ${req.params.id} has been marked as completed`,
      task: changeTask,
    });
...
async function deleteOneTask(req, res, next) {
  try {
    const oneTask = await tasksModel.findOneAndRemove({ id: req.params.id });
    res.json({ message: "Removed", removed: oneTask });
  } catch (error) {
    res.status(500).send(error);
  }
}
...
async function getAllTasks(req, res, next) {
  ...
  try {
    ...
    res.json({
      message: `Success, here are all ${allTasks.length} tasks!!`,
      Tasks: allTasks,
    });
  } catch (error) {
    res.status(500).send(error);
...
async function getOneTask(req, res, next) {
  ...
  try {
    ...
    res.json({ message: `Success, here's your task!'`, Tasks: oneTasks });
  } catch (error) {
    res.status(500).send(error);
  }
}
